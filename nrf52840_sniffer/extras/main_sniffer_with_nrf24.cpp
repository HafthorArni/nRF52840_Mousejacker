/*
 * nRF52840 Sniffer + nRF24L01+ Transmitter Test Bench
 * * This script runs on a single nRF52840 board.
 * - The nRF52840 (internal RADIO) acts as the "Sniffer".
 * - An attached nRF24L01+ (external) acts as a "Target" transmitter.
 * * The goal is to use the Sniffer UI to find and deconstruct the packets
 * being sent by its own nRF24.
 */

#include <Arduino.h>

// nRF24 (Sender) libraries
#include <SPI.h>
#include <nRF24L01.h>
#include <RF24.h>
...
// ------------------ Target TX (nRF24) Functions ------------------
/**
 * @brief Configures the external nRF24L01+ as the target transmitter.
 */
...
/**
 * @brief Non-blocking function to send a "Hello" packet every ~1s
 * from the external nRF24.
 */
...
void print_params() {
  ...
  if (preamble_trick_mode == 0) {
    ...
    if (discovered_count==0) Serial.println(F("(none)"));
      ...
      for (uint8_t i=0;i<discovered_count;i++){ Serial.print("0x"); Serial.print(discovered_base[i], HEX); Serial.print(i<discovered_count-1?" ":""); }
      Serial.println();
    }
  }
...
void discovery_undo_last_byte() {
  ...
  uint32_t base = 0;
  for (uint8_t i=0;i<discovered_count;i++) base |= ((uint32_t)discovered_base[i]) << (8*i);
      ...
      default:
        ...
        switch (c) {
          ...
          case 'b':
            ...
            break;
          case 'l':
            LFLEN = (LFLEN + 1) % 9; apply_pcnf0_pcnf1();
            Serial.print("LFLEN="); Serial.println(LFLEN);
            set_channel(best_ch_found); 
            break;
          case 's':
            S1LEN = (S1LEN + 1) % 8; apply_pcnf0_pcnf1();
...
void setup() {
  ...
  Serial.println("\n--- nRF52840 Sniffer + TX Test Bench ---");
  Serial.println("<<< nRF24 (TX) and nRF52 (Sniffer) running on same board >>>");

  // Configure the external nRF24L01+ as our target transmitter
...
void loop() {
  // 1. Run the target transmitter (non-blocking)
  run_target_transmitter(); 

  if (!tx_ok_logged) {
    char tx[TARGET_RADIO_PAYLOAD_SIZE] = {0};
    if (radio.write(&tx, 0)) { // Send a 0-byte test packet
      Serial.println("\n>>> TX DIAGNOSTIC: radio.write() Succeeded. Transmitter is likely working.\n");
      tx_ok_logged = true;
    }
  }

  // 2. Check for button press to start sweep
  ...
  // 3. Process serial commands from user
  serial_commands();

  // 4. Run the sniffer state machine
  switch (currentState) {
    case IDLE:
      delay(1);
      break;
